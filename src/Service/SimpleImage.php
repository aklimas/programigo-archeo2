...
class SimpleImage
{
    ...
    public function __destruct()
    {
        ...
        $type_check = ('object' == gettype($this->image) && 'GdImage' == get_class($this->image));

        if (null !== $this->image && is_resource($this->image) && $type_check) {
    ...
    public function fromFile($file)
    {
        ...
        if ('image/jpeg' === $this->mimeType && function_exists('exif_read_data')) {
    ...
    public function bestFit($maxWidth, $maxHeight)
    {
        ...
        if ('portrait' === $this->getOrientation()) {
            $height = $maxHeight;
    ...
    public function overlay($overlay, $anchor = 'center', $opacity = 1, $xOffset = 0, $yOffset = 0, $calculateOffsetFromEdge = false)
    {
        ...
        if (false !== strpos($anchor, 'top')) {
            $y = $yOffset;
        } elseif (false !== strpos($anchor, 'bottom')) {
            $y = $spaceY + ($calculateOffsetFromEdge ? -$yOffset : $yOffset);
        }

        // Determine if left|right
        if (false !== strpos($anchor, 'left')) {
            $x = $xOffset;
        } elseif (false !== strpos($anchor, 'right')) {
    ...
    public function text($text, $options, &$boundary = null)
    {
        ...
        if (true == $calculateOffsetFromEdge) {
            if (false !== strpos($anchor, 'bottom')) {
                $yOffset *= -1;
            }
            if (false !== strpos($anchor, 'right')) {
                $xOffset *= -1;
            }
        }
        ...
        if (true == $baselineAlign) {
            ...
            if (false !== strpos($anchor, 'bottom')) {
                $yOffset -= $boxFull[1];
            } elseif (false !== strpos($anchor, 'top')) {
    ...
    public function arc($x, $y, $width, $height, $start, $end, $color, $thickness = 1)
    {
        ...
        if ('filled' === $thickness) {
            ...
            $tempColor = (255 == self::normalizeColor($color)['red']) ? 'blue' : 'red';
    ...
    {
        $filter = 'gaussian' === $type ? IMG_FILTER_GAUSSIAN_BLUR : IMG_FILTER_SELECTIVE_BLUR;
    ...
    public function watermark_text($waterMarkText, $font = 4, $r = 158, $g = 158, $b = 158)
    {
    ...
    public function resizeImage($path, $filename, $sizes)
    {
        $names = [];
        $finder = new Finder();
        ...
        if ($finder->hasResults()) {
            foreach ($sizes as $size) {
                ...
                if (!file_exists($fullpath.'/'.$size.'x-'.$filename)) {
                    $image = new SimpleImage($fullpath.'/'.$filename);
                    $image->autoOrient();
                    $image->bestFit($size, $size);
                    $image->toFile($fullpath.'/'.$size.'x-'.$filename);
                    $names[] = $size.'x-'.$filename;
                }
            }
        }
        return $names;
