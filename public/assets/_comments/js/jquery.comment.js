...
(function ($, window, document, undefined) {
    ...
    var Comment = {
        ...
        buildForm_: function (comment_id, parent_id) {
            ...
            textarea.attr('id', 'newComment-'+parent_id);
            textarea.attr('placeHolder', 'Dodaj komentarz...');
            ...
            var button = $('<div class="float-end">Dodaj</div>');
            button.attr('id', 'add');

            button.addClass("btn btn-primary btn-sm mb-2 mt-2")


            button.on('click', function (e) {
                let commentText = $('#newComment-'+parent_id).val();
                if ($.trim(commentText).length > 0) {
                    self.submitForm_(comment_id, form_elem.serialize());
                }
            });

            form_elem.append(button);
                ...
                if (result.success != undefined) {
                    if (result.success === false) {
                        ...
                        if (comment_id != null)	// edit mode
                        {
                            ...
                            //self.$total_comment.html(self.options.title + ' ' + self.total_comment);
                           // self.total_comment++;
                        }
        ...
        buildItemList_: function (comment_info) {
            ...
            if (self.user_info_.is_edit_allowed && (comment_info.created_by == self.user_info_.user_id)) {
                ...
                var edit = $('<a>Edytuj</a>');
                edit.attr('href', '#');
                edit.attr('title', 'Edytuj');
                edit.attr('data-bs-toggle', 'tooltip');
                edit.attr('data-bs-placement', 'bottom');
            ...
            if (self.user_info_.is_edit_allowed && (comment_info.created_by == self.user_info_.user_id)) {
                ...
                var delete_ = $('<a>Usuń</a>');
                delete_.attr('href', '#');
                delete_.attr('title', 'Usuń');
                delete_.attr('data-bs-toggle', 'tooltip');
                delete_.attr('data-bs-placement', 'bottom');
            ...
            if (self.user_info_.is_add_allowed) {
                ...
                var reply = $('<a>Odpowiedz</a>');
                reply.attr('href', '#');
                reply.attr('title', 'Odpowiedz');
                reply.attr('data-bs-toggle', 'tooltip');
                reply.attr('data-bs-placement', 'bottom');
                reply.attr('data-bs-original-title', 'bottom');
        ...
        buildCountList_: function (total_comment) {
           ...
           // total = self.total_comment-1;
           // self.$total_comment.html(self.options.title+ ' ' +total);
        ...
        display_: function () {
            ...
            // poniższy kod słuzy do prezentacji łącznej liczby komentarzy (wystarczy go odkomentować)
...
function deleteComment(comment_id) {
    $('#deleteCommentBtn').attr('data-id', comment_id);
